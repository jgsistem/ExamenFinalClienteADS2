...
export class StudentService {
  API_URL : string = environment.apiUrl + 'api/Student';
  ...
  getStudentByType(type : string): void {
    this.httpClient.get<Student[]>(this.API_URL+'/GetIdStudent/'+type).subscribe(
      data => {
  ...
  getAllStudent(): void {
    this.httpClient.get<Student[]>(this.API_URL+'/GetAllStudent/').subscribe(
        data => {
          this.dataChange.next(data);
        },
        (error : any) => {
