...
export class ListComponent implements OnInit {
  ...
  displayedColumns = ['studentId', 'firstName', 'lastName', 'documentNumber', 'email','isActive','actions','monto'];
  ...
  onSubmit() {    
   ...
   if (this.studentType === 'PREGRADO'){
    this.studentType = '1';
   }

   if (this.studentType === 'MAESTRIA'){
    this.studentType = '2';
  }

  if (this.studentType === 'DOCTORADO'){
    this.studentType = '3';
  }
...
export class StudentDataSource extends DataSource<Student> {
  ...
  connect(): Observable<Student[]> {
        ...
        this.filteredData = this._studentDatabase.data.slice().filter((student: Student) => {
          const searchStr = (student.firstName + student.lastName + student.documentNumber).toLowerCase();
        ...
        return Observable.merge(...displayDataChanges).map(() => {
          ...
          this.filteredData = this._studentDatabase.data.slice().filter((student: Student) => {
            ...
            if(student === undefined || student.studentId === undefined){
              ...
              this.searchStr = (student.firstName + student.lastName + student.documentNumber).toLowerCase();
            }            
